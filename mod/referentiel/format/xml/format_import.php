...
class aformat_xml extends aformat_default {
    ...
    function provide_import() {
        return true;                  // Modif JF 2015/06/17
    }
    ...
    global $CFG;
		...
		if ($activite){
            ...
            $expout .= "<login>".referentiel_get_user_login($activite->userid)."</login>\n";
    ...
    function write_liste_activites() {
        ...
        // add comment
		//  MODIF JF 2015/06/17
		// Referentiel occurrence
        $expout .= "\n\n<!-- occurrence : ".$this->rreferentiel->id."  -->\n";
        $expout .= $this->write_occurrence();
        // add comment
	...
	/**
     * Turns occurrence referentiel into an xml segment
     * @param competence object
     * @return string xml segment
     */

    function write_occurrence() {   // only description data, no protocoel / domain / competencies / items
    	global $CFG;
        // initial string;
        $expout = "";
        // add comment
...
// MODIF JF 2012/03/26
			...
			// $expout .= "<id>$id</id>\n";
			$expout .= " <occurrence_id>$id</occurrence_id>\n";
            $expout .= " <occurrence_name>$name</occurrence_name>\n";
			$expout .= " <occurrence_code_referentiel>$code_referentiel</occurrence_code_referentiel>\n";
            $expout .= " <occurrence_description_referentiel>\n$description_referentiel</occurrence_description_referentiel>\n";
            $expout .= " <occurrence_cle_referentiel>\n$cle_referentiel</occurrence_cle_referentiel>\n";

            $expout .= " <occurrence_url_referentiel>$url_referentiel</occurrence_url_referentiel>\n";

            $expout .= " <occurrence_seuil_certificat>$seuil_certificat</occurrence_seuil_certificat>\n";
// MODIF JF 2012/03/26
            $expout .= " <occurrence_minima_certificat>$minima_certificat</occurrence_minima_certificat>\n";
            $expout .= " <occurrence_timemodified>$timemodified</occurrence_timemodified>\n";
            $expout .= " <occurrence_nb_domaines>$nb_domaines</occurrence_nb_domaines>\n";
            $expout .= " <occurrence_liste_codes_competence>$liste_codes_competence</occurrence_liste_codes_competence>\n";
            $expout .= " <occurrence_liste_empreintes_competence>$liste_empreintes_competence</occurrence_liste_empreintes_competence>\n";
			// $expout .= " <occurrence_local>$local</occurrence_local>\n";
			// PAS DE LOGO ICI
			// $expout .= " <occurrence_logo_referentiel>$logo_referentiel</occurrence_logo_referentiel>\n";
        }
        return $expout;
    }
	 ...
	 * modifie la base de donnees
     */
    function import_activities( $xmlreferentiel ) {
	// recupere le fichier xml
	// ne cr�e ni ne modifie JAMAIS l'occurrence
	//
	global $SESSION;
	global $USER;
	global $CFG;
	$nbactivities=0;        // compteur
	//$nbcompetences=0;        // compteur
    //$nbitems=0;              // compteur
    ...
    $error_nocode = get_string( 'xmlimportnocode', 'referentiel' );
	$error_override = get_string( 'overriderisk', 'referentiel' );
    $error_notsameoccurrence = get_string( 'notsameoccurrence', 'referentiel' );
    $error_notsameinstance = get_string( 'notsameinstance', 'referentiel' );

    $occurrence = $this->defaultoccurrence();
	$occurrence->id = $this->getpath( $xmlreferentiel, array('#','occurrence_id',0,'#'), '', false, '');
    $occurrence->name = $this->getpath( $xmlreferentiel, array('#','occurrence_name','0','#'), '', true, $error_noname);
    $occurrence->code_referentiel = $this->getpath( $xmlreferentiel, array('#','occurrence_code_referentiel',0,'#'), '', true, $error_nocode);
    $occurrence->description_referentiel = $this->getpath( $xmlreferentiel, array('#','occurrence_description_referentiel',0,'#','text',0,'#'), '', true, '');
    $occurrence->cle_referentiel = $this->getpath( $xmlreferentiel, array('#','occurrence_cle_referentiel',0,'#','text',0,'#'), '', true, '');
	$occurrence->liste_codes_competence = $this->getpath( $xmlreferentiel, array('#','occurrence_liste_codes_competence',0,'#'), '', true, '');

	// Verify compatibility
	if (empty($this->rreferentiel){
			|| !(($this->rreferentiel->code_referentiel == $occurrence->occurrence_code_referentiel)
				&& ($this->rreferentiel->cle_referentiel == $occurrence->occurrence_cle_referentiel)
				&& ($this->rreferentiel->liste_codes_competence == $occurrence->occurrence_liste_codes_competence)))  {
		// ni nouvelle instance ni recouvrement
		$this->error( $error_notsameoccurrence );
		return false;
	}

	// on ne tient pas compte de l'instance du fichier car c'est l'instance courante qu'on charge.
    if (empty($this->ireferentiel) {
		$this->error( $error_notsameinstance );
		return false;
    }
	...
	$xmlactivites = $xmlreferentiel['#']['activite'];

	$aindex=0;

	$nbactivites=0;   // compteur
    foreach ($xmlactivites as $activite) {
		// activiteS
		// print_r($activite);
		$new_activite = $this->defaultactivite();

		//$new_activite->id =$this->getpath( $activite, array('#','id',0,'#'), '', false, '');

		$new_activite->type_activite=$this->getpath( $activite, array('#','type_activite',0,'#'), '', true, '');;
		$new_activite->description_activite=$this->getpath( $activite, array('#','description_activite',0,'#','text',0,'#'), '', true, '');
		$new_activite->competences_activite=$this->getpath( $activite, array('#','competences_activite',0,'#','text',0,'#'), '', true, '');
        $new_activite->commentaire_activite=$this->getpath( $activite, array('#','commentaire_activite',0,'#','text',0,'#'), '', true, '');
    	$new_activite->ref_instance=$this->ireferentiel->id;
    	$new_activite->ref_referentiel=$this->rreferentiel->id;
	    $new_activite->ref_course=$this->course->id;

		// $new_activite->userid=$this->getpath($activite, array('#','userid',0,'#'), '', false, ''); // On utilise seulement les nom et prenom
	    $user_login=$this->getpath( $activite, array('#','login',0,'#','text',0,'#'), '', true, '');
	    $user_lastname=$this->getpath( $activite, array('#','lastname',0,'#','text',0,'#'), '', true, '');
	    $user_firstname=$this->getpath( $activite, array('#','firstname',0,'#','text',0,'#'), '', true, '');
	    $teacherid=$this->getpath($activite, array('#','teacherid',0,'#'), '', false, '');
	    $teacher_lastname=$this->getpath( $activite, array('#','teacher_lastname',0,'#','text',0,'#'), '', true, '');
	    $teacher_firstname$this->getpath( $activite, array('#','teacher_firstname',0,'#','text',0,'#'), '', true, '');

		$new_activite->date_creation=$this->getpath( $activite, array('#','date_creation',0,'#','text',0,'#'), '', true, '');
	    $new_activite->date_modif_student=$this->getpath( $activite, array('#','date_modif_student',0,'#','text',0,'#'), '', true, '');
	    $new_activite->date_modif=$this->getpath( $activite, array('#','date_modif',0,'#','text',0,'#'), '', true, '');
    	$new_activite->approved=$this->getpath( $activite, array('#','approved',0,'#'), '', false, '');

		// retablir des sauts de ligne
        $new_activite->description_activite=preg_replace("/\|\|\|/", "\r\n" , $new_activite->description_activite);
        $new_activite->commentaire_activite=preg_replace("/\|\|\|/", "\r\n" , $new_activite->commentaire_activite);

		// Verifier que l'utilisateur existe et est inscrit dans ce cours, sinon on ne cr�e pas l'activit�
        $new_activite->userid = 0; //

		if ($userid = referentiel_get_userid_by_login($user_login)){
			if ( referentiel_is_student_course($this->course->id, $userid){
				$new_activite->userid = $userid;
			}
		}
		// If not the same login test firstname / lastname
		if ((!$new_activite->userid) && ($users = referentiel_get_user_by_firstname_lastname($user_firstname, $user_lastname))){
			foreach ($users as $user){
				if ( referentiel_is_student_course($this->course->id, $user->id){
					$new_activite->userid = $user->id;
				}
			}
		}

		// Verifier que l'utilisateur existe et est inscrit dans ce cours, sinon on ne cr�e pas l'activit�
        $new_activite->teacherid = 0;
		if ($teachers = referentiel_get_user_by_firstname_lastname($teacher_firstname, $teacher_lastname)){
			foreach ($teachers as $teacher){
				if ( referentiel_is_teacher_course($this->course->id, $teacher->id){
					$new_activite->teacherid = $teacher->id;
				}
			}
		}
		...
		// enregistrer
        if ($new_activite->userid){
            $re->activites[$aindex]=$new_activite;
        	$aindex++;
			// sauvegarder dans la base
			// remplacer l'id du referentiel importe par l'id du referentiel cree
			// trafiquer les donnees pour appeler la fonction ad hoc

			$new_activite_id=referentiel_add_activite($new_activite);    // A verifier
			if ($new_activite_id){
                $nbactivites++;
            }
...
// retablir des sauts de ligne
     ...
     * @return array (of objects) question objects
     */
    function read_import_activities($lines) {
        // we just need it as one big string
        $text = implode($lines, " ");
        unset( $lines );
		...
		// echo "<br /><br />\n";
		// exit;
		$re=$this->import_activities($xml['activities']);
        // stick the result in the $treferentiel array
 		// DEBUG
		// echo "<br />DEBUG xml/format.php :: ligne 632\n";
		// print_r($re);

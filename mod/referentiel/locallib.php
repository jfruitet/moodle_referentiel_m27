...
// -------------------------------------
function referentiel_shortstring($s, $lmax=512){
    if ($s){
        $l=mb_strlen($s, 'utf-8');
        if ($l>$lmax){
            return mb_strimwidth($s, 0, $lmax, ' (...) ', 'utf-8');
        }
    }
    return $s;
}
...
/**
 * This function return an short string
 *
 * @param string
...
$ok=true;
	...
	// supprimer le bareme
	$DB->delete_records("referentiel_a_scale_ref", array("refrefid" =>"$id"));

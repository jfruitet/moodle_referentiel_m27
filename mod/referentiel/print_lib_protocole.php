...
static $referentiel_id = NULL;
	...
	if (!empty($referentiel_instance->ref_referentiel)){
		...
		if ($isteacher || $istutor || $iseditor){
            echo referentiel_affiche_select_protocole($referentiel_instance->id, $referentiel_instance->ref_referentiel, $course->id, $mode, $select_acc);
		}
		else{
            echo referentiel_affiche_protocole($referentiel_instance->ref_referentiel);
        }
	}
	echo '<br /><br />'."\n";
	return true;
...
// tables de protocoles (seuils, item obligatoires, etc.
    ...
    // recupere les labels des domaine, competence, item
